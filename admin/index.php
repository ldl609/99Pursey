<html>
<head>
<link href="../css/style.css" rel="stylesheet">
<meta http-equiv="Content-Type" content="text/html; charset=gb2312">
<title>九九度供求信息网--后台管理系统</title>
</head>
<frameset rows="*" cols="1*,1045,1*" framespacing="0" frameborder="NO" border="0">
  <frame src="blank.php" name="left" scrolling="NO" noresize>

  <frameset rows="2000,*" cols="*" framespacing="0" frameborder="NO" border="0">

		<frameset rows="110,*,80" cols="*" framespacing="0" frameborder="NO" border="0">
		  <frame src="top.php" name="topFrame" scrolling="NO" noresize>
			 <frameset  rows="*" cols="250,*"  framespacing="0" frameborder="NO" border="0">
				<frame src="left.php" name="leftFrame" scrolling="NO" noresize>
				<frame src="main.php" name="mainFrame" scrolling="" noresize>
			</frameset>
		  <frame src="bottom.php" name="bottomFrame" scrolling="NO" noresize>
		</frameset>

		<frame src="blank.php" name="right" scrolling="NO" noresize>

	</frameset>

<frame src="blank.php">
</frameset>
<noframes>
<body>
</body>
</noframes>
</html>
